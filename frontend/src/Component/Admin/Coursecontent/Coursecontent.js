...
const Coursecontent = () => {
  ...
  useEffect(() => {
    axios
      .get("http://192.168.252.191:5000/course/getcourse")
      .then((res) => {
  ...
  useEffect(() => {
    if (selectedCourse) {
      axios
        .get(`http://192.168.252.191:5000/course/getmodules/${selectedCourse}`)
        .then((res) => {
          setModules(res.data || []);
        })
        .catch((error) => {
  ...
  const handleSubmit = (event) => {
    ...
    axios
      .post("http://192.168.252.191:5000/course/submitcon", formData)
      .then((response) => {
